...
impl DualContext {
    ...
    async fn new() -> Self {
        ...
        // This ensures Mollusk uses the EXACT same background stake for identical history
...
// Including large numbers representing years of operation (assuming ~182 epochs/year @ 2 days/epoch)
// Also tests stake history depth limit (512 epochs) to ensure old entries drop correctly
#[test_case(5 ; "five_epochs")]
#[test_case(10 ; "ten_epochs")]
#[test_case(20 ; "twenty_epochs")]
#[test_case(50 ; "fifty_epochs")]
#[test_case(100 ; "hundred_epochs")]
#[test_case(182 ; "one_year")]
#[test_case(500 ; "five_hundred_epochs")]
#[test_case(512 ; "stake_history_depth_boundary")]
#[test_case(515 ; "beyond_stake_history_depth")]
#[test_case(1800 ; "ten_years")]
#[tokio::test]
async fn test_single_delegation_activation(num_epochs: u64) {
    ...
    for i in 0..num_epochs {
        ...
        if num_epochs >= 100 && (i % 100 == 0 || i == num_epochs - 1) {
...
// Test various stake amounts: minimum, small, large
...
#[tokio::test]
async fn test_stake_amounts_activation(staked_amount: u64) {
    let mut ctx = DualContext::new().await;
...
// Test deactivation with various stake amounts to verify warmup rates match during cooldown
#[test_case(MINIMUM_DELEGATION ; "minimum_delegation")]
#[test_case(MINIMUM_DELEGATION * 100 ; "small_amount")]
#[test_case(250_000 * 1_000_000_000 ; "large_amount")]
#[tokio::test]
async fn test_stake_amounts_deactivation(staked_amount: u64) {
    let mut ctx = DualContext::new().await;
...
async fn test_multiple_simultaneous_delegations(num_stakes: usize, num_vote_accounts: usize) {
    ...
    for _ in 1..num_vote_accounts {
        ...
        vote_accounts.push((vote_account, vote_account_data));
    }
    ...
    for i in 0..num_stakes {
        ...
        // Delegate to vote account (alternate if multiple)
...
async fn test_staggered_delegations_over_epochs() {
    ...
    for (idx, i) in [0, 5, 10, 15, 20].iter().enumerate() {
        ...
        ctx.fund_stake_account(&stakes[idx], staked_amount).await;
...
async fn test_mixed_lifecycle_stress() {
    ...
    for _ in 0..total_stakes {
        ...
        let staked_amount = MINIMUM_DELEGATION;
        stake_account.set_lamports(stake_account.lamports() + staked_amount);
        ctx.fund_stake_account(&stake, staked_amount).await;
...
#[tokio::test]
async fn test_delegate_after_full_deactivation_different_validator() {
    let mut ctx = DualContext::new().await;

    let vote_account_b = Pubkey::new_unique();
    let vote_account_b_data = create_vote_account();
    ctx.program_test_ctx
        .set_account(&vote_account_b, &vote_account_b_data.clone());
...
#[tokio::test]
async fn test_delegate_after_deactivation_during_warmup() {
    let mut ctx = DualContext::new().await;
...
#[tokio::test]
async fn test_delegate_during_deactivation_cooldown() {
    let mut ctx = DualContext::new().await;
    ...
    assert_eq!(
        banks_effective, mollusk_effective,
        "Effective stake mismatch immediately after delegating during deactivation"
    );
...
#[tokio::test]
async fn test_old_warmup_rate_activation() {
    let mut ctx = DualContext::new().await;
...
#[tokio::test]
async fn test_old_warmup_rate_deactivation() {
    let mut ctx = DualContext::new().await;
...
#[tokio::test]
async fn test_massive_stake_relative_to_background() {
    let mut ctx = DualContext::new().await;
    ...
    for epoch_offset in 0..epochs_to_activate {
        let epoch = ctx.mollusk.sysvars.clock.epoch - epochs_to_activate + epoch_offset;
        ctx.verify_background_stake_preservation(epoch, background_effective)
            .await;
    }
}

#[tokio::test]
async fn test_massive_stake_deactivation() {
    let mut ctx = DualContext::new().await;
    ...
    for epoch_offset in 0..epochs_to_deactivate {
        let epoch = ctx.mollusk.sysvars.clock.epoch - epochs_to_deactivate + epoch_offset;
        ctx.verify_background_stake_preservation(epoch, background_effective)
            .await;
    }
}
