...
fn move_stake_or_lamports_shared_checks(
    ...
    // runtime guards against unowned writes, but MoveStake and MoveLamports are
    // defined by SIMD we check explicitly to avoid any possibility of a
    // successful no-op that never attempts to write
