...
use {
    helpers::{
        ...
        instruction_builders::{MoveStakeConfig, MoveStakeWithVoteConfig},
...
fn test_move_stake(
    ...
    if ctx.minimum_delegation.unwrap() > 1 {
...
fn test_move_stake_general_fail(move_source_type: StakeLifecycle, move_dest_type: StakeLifecycle) {
    ...
    if move_dest_type == StakeLifecycle::Active {
        ...
        let vote_account_data = ctx.vote_account_data.as_ref().unwrap();
